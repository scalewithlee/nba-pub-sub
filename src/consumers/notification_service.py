...
class NotificationService:
    def __init__(self, project_id: str, subscription_name: str, failure_rate: float = 0.4):
    ...
    def process_message(self, message: pubsub_v1.subscriber.message.Message):
        ...
        try:
            ...
            if random.random() < self.failure_rate:
                self.failed_count += 1
                print(f"Notification FAILED for: {player} {event} (failure #{self.failed_count})")
            ...
            print(f"Notification sent: {player} {event} (success #{self.processed_count})")
            message.ack()
    ...
    def start_listening(self, timeout: int = 30):
        ...
        print(f"Notification Service listening (with {self.failure_rate*100}% failure rate)...")
